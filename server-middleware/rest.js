...
const CERT = fs.readFileSync(
  `./keys/${process.env.PASS_TYPE_IDENTIFIER.substring(5)}.pem`
);
const CERT_PASSPHRASE = process.env.PASS_CERT_SECRET;
const APPLE_CA_CERTIFICATE = forge.pki.certificateFromPem(
  fs.readFileSync("./keys/wwdr.pem")
);

const FAQ_DATA_URL = {
  DE: process.env.FAQ_DATA_URL_DE,
  EN: process.env.FAQ_DATA_URL_EN,
};

// env variable FAQ_CACHE_DURATION defines expiration time of cache in seconds, set 0 if cache should not expire
const FAQ_CACHE_DURATION =
  process.env.FAQ_CACHE_DURATION && !isNaN(process.env.FAQ_CACHE_DURATION)
    ? parseInt(process.env.FAQ_CACHE_DURATION) * 1000
    : 60 * 60 * 1000;
...
app.get("/faq", async (req, res) => {
  ...
  if (!(locale in faqCache) || isExpired(faqCache[locale].added)) {
    try {
      ...
      console.error(
        `Could not fetch FAQ data for locale ${locale}, falling back to cache`
      );
    }
  }
    ...
    res
      ...
      .send([
        locale === "de"
          ? {
              q: "Fehler",
              a: "Da ging etwas schief, versuche es später erneut",
            }
